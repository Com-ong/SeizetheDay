...
public class GuestMgrPool {
	...
	public Vector<GuestBean> getEachGuestList(int exhibition_seq)
	{
		Connection conn = null;
		Statement stmt = null;
		ResultSet rs = null;
		...
		try {
			conn = pool.getConnection();
			String strQuery = "select * from guest where EXHIBITION_SEQ = '" + exhibition_seq + "'";
			stmt = conn.createStatement();
			rs = stmt.executeQuery(strQuery);
	...
	public void insertGuest(/*int board_seq, */int user_seq, String board_text, Date board_date, int exhibition_seq)
	{
		...
		try {
			...
			String strQuery = "insert into guest (USER_SEQ, BOARD_TEXT, BOARD_DATE, EXHIBITION_SEQ) values (?, ?, ?, ?)";
			pstmt = conn.prepareStatement(strQuery);
			//pstmt.setInt(1, board_seq);
			pstmt.setInt(1, user_seq);
			pstmt.setString(2, board_text);
			pstmt.setDate(3, board_date);
			pstmt.setInt(4, exhibition_seq);
